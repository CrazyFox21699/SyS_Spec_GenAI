...
def test_gpt_gen_logic_merged_cell_evidence() -> None:
    ...
    path = root / "sample_inputs" / "input" / "GPT_GenLogic.xlsx"
    if not path.exists():
        path = root / "sample_inputs" / "GPT_GenLogic.xlsx"
    if not path.exists():
