...
def _plan_prompt(context_pack: dict[str, Any], engineer_note: str) -> str:
    ...
    return (
        ...
        "FOOTNOTE / CROSS-SPEC RULES (critical for Japanese specs with (*n) markers):\n"
        "- Conditions like 'RDY signal reception control = start *1' reference footnotes or OTHER spec sections.\n"
        "- Use footnote_knowledge.footnotes and attached_logic_blocks for resolved meaning — NEVER invent values.\n"
        "- If footnote_knowledge.unresolved_footnote_count > 0 or cross_refs are unresolved, add open_questions.\n"
        "- Do NOT hardcode footnote expansions; ask the engineer via open_questions when evidence is missing.\n"
