...
def build_workspace_payload(
    ...
    *,
    ...
    try:
        from web.testcase_group_mapping import build_group_mapping, enrich_rows_with_group_id, load_group_mapping
        group_mapping = build_group_mapping(raw_rows)
        # Merge user-edited fields (fixture_class, namespace, main_function) from persisted mapping.
        # build_group_mapping always auto-suggests fresh values; persisted mapping has user edits.
        # Without this merge, workspace refresh overwrites user fixture edits with auto-suggestions.
        if job_output_dir is not None:
            try:
                _persisted = load_group_mapping(job_output_dir)
                _persisted_groups = _persisted.get("groups") or {}
                if _persisted_groups:
                    for _gid, _grp in (group_mapping.get("groups") or {}).items():
                        _p = _persisted_groups.get(_gid)
                        if _p:
                            for _f in ("suggested_fixture_class", "suggested_namespace", "default_main_function"):
                                if _f in _p:
                                    _grp[_f] = _p[_f]
            except Exception:
                pass
        workbench_rows = enrich_rows_with_group_id(raw_rows, group_mapping)
    except Exception:
        group_mapping = {}
        workbench_rows = raw_rows
...
def persist_generated_draft_workflow(
    ...
    *,
    ...
    # API-generated code must not auto-SAVED — requires explicit user approval.
    if not allow_auto_save and code_status == "SAVED":
        code_status = "NEEDS_REVIEW"
        wf = {**wf, "code_status": "NEEDS_REVIEW", "workflow_status": "NEEDS_REVIEW",
              "workflow_message": wf.get("workflow_message") or "generated by API — awaiting review"}
    ...
    # When real TEST_F code was generated, clear stale missing-context markers from prior failures.
    # draft_payload may carry these fields from the previous MISSING_CONTEXT or UNRESOLVED run.
    _has_testf = bool(re.search(r"\bTEST(?:_F)?\s*\(", prepared))
    _raw_missing_count = len(draft_to_save.get("missing_context") or [])
    if _has_testf:
        for _stale_key in ("missing_context", "missing_information", "unresolved_message", "unresolved_copilot_reason"):
            draft_to_save.pop(_stale_key, None)
        # If issue_reason was carried from a previous MISSING_CONTEXT or UNRESOLVED run, replace it
        if draft_to_save.get("issue_reason") in ("missing_generation_context", "unresolved_by_copilot"):
            draft_to_save["issue_reason"] = _issue_reason
    ...
    return {
        **wf,
        "candidate_id": cid,
        "draft": saved,
        "has_testf_code": _has_testf,
        "raw_missing_count": _raw_missing_count,
        "filtered_missing_count": 0 if _has_testf else _raw_missing_count,
    }
