...
def _candidate_row(
    ...
    *,
    ...
    use_case = _lang_text(overlay, "use_case", language) or str(candidate.get("use_case_description") or "")
    operation = _lang_text(overlay, "operation", language) or _join_when(candidate)
    expected_input = _lang_text(overlay, "expected_input", language) or materialize_expected_input(
        candidate, definition_lookup
    )
    ...
    if not operation:
        operation = _join_preconditions(candidate)
    if not operation:
        operation = _transition_operation(binding)
    trace = candidate.get("traceability") or {}
    evidence: list[str] = []
